...
/**
 * Array of fumidai server addresses with ports
 * Format: ['hostname:port', 'hostname:port']
 */
export const fumidais = ['cdn.xn--b6gac.eu.org:443', 'cdn-all.xn--b6gac.eu.org:443'];

/**
 * Default SOCKS5 fumidai configuration
...
/**
 * Default fumidai connection timeout in milliseconds
 * Used when rotating through multiple fumidai addresses
 */
export const defaultFumidaiTimeout = 1500;

/**
 * Default fumidai fallback setting
 * When true: Falls back to direct connection if all proxies fail
 * When false: Fails immediately if all proxies fail
 */
export const defaultEnableFumidaiFallback = true;
 ...
 * @param {string} env.SOCKS5 - SOCKS5 fumidai configuration
 ...
 * @param {string} env.FUMIDAI_TIMEOUT - Fumidai connection timeout in ms
 * @param {string} env.FUMIDAI_FALLBACK - Enable fallback to direct connection
 * @param {string} env.VLESS_OUTBOUND - VLESS outbound configuration URL
 * @returns {Object} Request configuration
 */
export function createRequestConfig(env = {}) {
	const { UUID, SOCKS5, SOCKS5_RELAY, TROJAN_PASSWORD, FUMIDAI_TIMEOUT, FUMIDAI_FALLBACK, VLESS_OUTBOUND } = env;
	...
	return {
		...
		fumidai: null,
		fumidaiPort: null,
		// Fumidai type: 'socks5' | 'http' | 'vless' | null
		fumidaiType: null,
		parsedFumidaiAddress: null,
		// Multi-fumidai rotation settings
		fumidaiTimeout: FUMIDAI_TIMEOUT ? parseInt(FUMIDAI_TIMEOUT, 10) : defaultFumidaiTimeout,
		enableFumidaiFallback: FUMIDAI_FALLBACK !== 'false' && defaultEnableFumidaiFallback,
