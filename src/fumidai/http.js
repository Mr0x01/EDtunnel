/**
 * HTTP CONNECT fumidai implementation
 */

const authorizationHeaderName = atob('UHJveHktQXV0aG9yaXphdGlvbjo=');

/**
 * Establishes HTTP CONNECT tunnel fumidai connection.
 ...
 * @param {{username?: string, password?: string, hostname: string, port: number}} parsedHttpAddr - Parsed HTTP fumidai address
...
export async function httpConnect(addressType, addressRemote, portRemote, log, parsedHttpAddr, connect, initialData = new Uint8Array(0)) {
	...
	// Connect to HTTP fumidai server
	...
	try {
		...
		const auth = username && password ? `${authorizationHeaderName} Basic ${btoa(`${username}:${password}`)}\r\n` : '';
		...
		log(`HTTP fumidai response: ${headerText.split('\r\n')[0]}`);
