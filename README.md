...
A fumidai tool based on Cloudflare Workers and Pages, supporting multiple protocols and configuration options.
...
- Custom fumidai IP and port support
- SOCKS5 and HTTP fumidai support
- **Trojan protocol support** with auto-detection
- **VLESS outbound fumidai** with full UDP capability
- **Multi-fumidai rotation** with automatic failover
- Automatic configuration subscription link
- URL query parameter configuration override support
- Path-based fumidai parameters (`/socks5://`, `/http://`, `/vless://`)
...
| `FUMIDAI` | No | `1.1.1.1` or `example.com`<br>Multiple: `1.1.1.1:9443,2.2.2.2:8443` | Custom fumidai IP and port |
| `SOCKS5` | No | `user:pass@host:port`<br>Multiple: `user1:pass1@host1:port1,user2:pass2@host2:port2` | SOCKS5 fumidai configuration |
...
| `VLESS_OUTBOUND` | No | `vless://uuid@host:port?type=ws&security=tls` | VLESS outbound fumidai URL |
| `FUMIDAI_TIMEOUT` | No | `1500` | Fumidai connection timeout in ms (default: 1500) |
| `FUMIDAI_FALLBACK` | No | `true` or `false` | Fallback to direct connection if proxies fail (default: true) |
...
|-----------------|-------------------|---------|-------------|
| `fumidai` | `FUMIDAI` | `?fumidai=1.1.1.1:443` | Override fumidai IP and port |
| `socks5` | `SOCKS5` | `?socks5=user:pass@host:port` | Override SOCKS5 fumidai configuration |
| `http` | - | `?http=user:pass@host:port` | HTTP CONNECT fumidai configuration |
| `vless` | `VLESS_OUTBOUND` | `?vless=vless://uuid@host:port` | Override VLESS outbound fumidai |
| `globalfumidai` | - | `?globalfumidai` | Enable global fumidai mode (route all traffic) |

### Path-Based Fumidai Parameters

You can also configure proxies directly in the URL path:

| Path Format | Example | Description |
|-------------|---------|-------------|
| `/fumidai=` | `/fumidai=1.1.1.1:443` | Set fumidai IP via path |
| `/socks5://` | `/socks5://user:pass@host:port` | SOCKS5 fumidai via path |
| `/http://` | `/http://user:pass@host:port` | HTTP CONNECT fumidai via path |
...
1. Temporarily change fumidai IP:
   ```
...
- If you see URLs like `/%3Ffumidai=value`, this won't work correctly. Use `/?fumidai=value` instead.
...
1. Visit `https://fumidai.edtunnel.best/`
2. Enter `Fumidai:fumidaiport` and click Check
3. When showing `Fumidai IP: true` it's available
4. Configure in Worker: `FUMIDAI=211.230.110.231:50008`

Note: Fumidai IPs with ports may not work on HTTP-only Cloudflare sites.
...
### HTTP Fumidai Support

As an alternative to SOCKS5, you can use HTTP CONNECT fumidai:
...
### SOCKS5 Fumidai Configuration

Supports the following formats:

- Basic format: `host:port`
- Authentication format: `username:password@host:port`
- Multiple proxies (separated by English comma): `fumidai1,fumidai2,fumidai3`
...
### Multi-Fumidai Rotation and Fallback

When configuring multiple fumidai addresses, the system provides:

- **Random rotation**: Automatically selects from available proxies
- **Connection timeout**: Configurable via `FUMIDAI_TIMEOUT` (default: 1500ms)
- **Automatic failover**: Tries next fumidai on failure
- **Direct fallback**: Falls back to direct connection if all proxies fail (configurable via `FUMIDAI_FALLBACK`)
...
- Ensure fumidai servers are stable and available
- Recommend using private proxies for better security
- Use commas to separate multiple proxies
- Support dynamic fumidai addition and removal

## Notes

- Fumidai IPs with ports may not work on HTTP-only Cloudflare sites
